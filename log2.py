...
metal_hud_pattern = re.compile(r'metal-HUD: ([\d\.,]+)')
...
syslog = SyslogServer(lockdown=rsd)

def get_log_data():
        
        d = syslog.c.recv(4096)
        d = d.decode('utf-8')
...
def write_to_file(file_name):
    while True:
        try:
            ...
            matches = metal_hud_pattern.findall(data)  
            with file_lock: 
...
if __name__ == '__main__':
    ...
    #window.show()
    #sys.exit(app.exec_())
    read_thread = threading.Thread(target=read_from_io)
    write_threads = [
        threading.Thread(target=write_to_file, args=("./log_test.txt",))
        for _ in range(3)
    ]
